...
function createMenuItem(dataUrl, menuName) {
    ...
    $('.menuTab', topWindow).each(function() {
        if ($(this).data('id') == dataUrl) {
            if (!$(this).hasClass('active')) {
                ...
                // 显示tab对应的内容区
                $('.mainContent .JeeOpen_iframe', topWindow).each(function() {
                    if ($(this).data('id') == dataUrl) {
    ...
    if (flag) {
        ...
        $('.mainContent', topWindow).find('iframe.JeeOpen_iframe').hide().parents('.mainContent').append(str1);
